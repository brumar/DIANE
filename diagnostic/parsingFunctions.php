...
function findWords($text,$words){//find words in a text (with spelling mistakes) and output detailed informations about the mistakes
	...
	foreach ($words as $word){
		...
		if(strpos($word, "|")){ // S'il y a plusieurs alternatives, on les observe une par une jusqu'à tomber sur une
			foreach(explode("|", $word) as $w){
				$pattern="#". preg_quote($w)."#";
				$try = FindBestPattern($text,$pattern,$w);
				if($try['found']){
					$output[$w]=$try;
					break;
				}
...
function PickBest($text,$tabInput,&$log = null){
	// Maintenant : $tabInput contient une liste d'array contenant 1) l'id, 2) les wordsArrays d'avant
	if($log){$log.="<u>R&eacuteponse analys&eacutee : </u><br><i>$text</i></br>";}

	$min=-INF;
	$output=array();
	$notes=array();

	foreach($tabInput as $tab){
		$wordsArray = $tab['words'];
		if($log){$keys=implode(' ; ',$wordsArray);$log.="<u>Mots clefs propos&eacutes : </u><br><i>$keys</i></br>";}
		if($log){$log.="<u>Analyse</u>";}
		$note=getNote($text,$wordsArray,$log);
		if($note>$min){$min=$note;$output=$tab; }
	}
	return array("eval"=>$min,"words"=>$output["words"][0], "id"=>$output["id"]);
}
